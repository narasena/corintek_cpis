...
type FormatLimitParam = Pick<
  TPreviewParameter,
  'minValue' | 'maxValue' | 'unit' | 'valueType' | 'category' | 'variableName'
>;

type FormatRawWaterLimitParam = Pick<
  TPreviewParameter,
  'rawWaterMinValue' | 'rawWaterMaxValue' | 'unit'
>;

function formatLimit(parameter: FormatLimitParam): string {
...
function formatRawWaterLimit(parameter: FormatRawWaterLimitParam): string {
...
function formatValue(state?: TEntryState): string {
  if (!state) return '';
...
): { id: string; unitNumber: number; type: 'CHILLER' | 'COOLING_TOWER' }[] {
  if (!machines) return [];
