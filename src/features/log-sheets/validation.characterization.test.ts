...
type Machine = { id: string; unitNumber: number; type: 'CHILLER' | 'COOLING_TOWER' };

function baseInput(overrides?: Partial<TLogSheetValidationInput>): TLogSheetValidationInput {
  const chillers: Machine[] = [{ id: 'c1', unitNumber: 1, type: 'CHILLER' }];
  const coolingTowers: Machine[] = [{ id: 't1', unitNumber: 1, type: 'COOLING_TOWER' }];
...
describe('validateLogSheetEntries (characterization)', () => {
  ...
  it('requires at least one active chiller when there are chillers (submit-blocking path)', () => {
    ...
    expect(result.valid).toBe(false);
    expect(result.errors).toContain('Minimal satu Chiller harus dipilih dan diisi lengkap.');
  });
  ...
  it('collects missing fields from the first active chiller if none are complete (surprising/buggy-ish: only first machine detailed)', () => {
    ...
    expect(
      result.missingFields.some(s => s.includes('Chiller #1'))
    ).toBe(true);
    expect(result.errors).toContain('Minimal satu Chiller harus diisi lengkap.');
  });
  ...
  it('TEXT parameters are considered missing when no entryState exists for them (surprising behavior)', () => {
    ...
    const result = validateLogSheetEntries(input);
    expect(result.missingFields.some(s => s.includes('JOB_DESCRIPTION'))).toBe(true);
  });
  ...
  it('raw water missing is collected from COOLING_WATER_QUALITY excluding variableName containing "cycle" (main path)', () => {
    ...
    expect(
      result.missingFields.some(s => s.includes('Raw Water Quality') && s.toLowerCase().includes('cycle'))
    ).toBe(false);
  });
