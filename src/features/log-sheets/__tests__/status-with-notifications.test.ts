...
function createDetail(assignments: ILogSheetDetailView['project']['assignments']) {
  return {
    logSheet: {
      id: 'ls-1',
      projectId: 'proj-1',
...
describe('updateLogSheetStatusWithNotifications', () => {
  ...
  it('delegates directly to updateLogSheetStatus when status is not SUBMITTED', async () => {
    ...
    (serviceMock.updateLogSheetStatus as unknown as ReturnType<typeof vi.fn>).mockResolvedValue(
      logSheet
    );
    ...
    expect(notificationsMock.notifyLimitBreachesOnSubmission).not.toHaveBeenCalled();
  ...
  it('validates, notifies, and updates status when submitting', async () => {
    ...
    (serviceMock.getLogSheetDetail as unknown as ReturnType<typeof vi.fn>).mockResolvedValue(
      detail
    );
    const logSheet = createLogSheet();
    (serviceMock.updateLogSheetStatus as unknown as ReturnType<typeof vi.fn>).mockResolvedValue(
    ...
    expect(serviceMock.validateLogSheetForSubmission).toHaveBeenCalledWith('ls-1');
    expect(serviceMock.getLogSheetDetail).toHaveBeenCalledWith('ls-1');
    expect(notificationsMock.notifyLimitBreachesOnSubmission).toHaveBeenCalledWith({
  ...
  it('propagates errors from validateLogSheetForSubmission', async () => {
    ...
    (serviceMock.validateLogSheetForSubmission as unknown as ReturnType<typeof vi.fn>).mockRejectedValue(
      error
    );
