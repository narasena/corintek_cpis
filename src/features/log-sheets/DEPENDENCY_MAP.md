...
> Generated: 2026-02-14 | Scope: `src/app/(main)/log-sheets/**` + `src/features/log-sheets/**`

---

## 1. File Inventory (25 files)
...
| A6  | `[projectId]/components/log-sheet-form.tsx`                       |      195 | Create log-sheet form                    |
| A7  | `[projectId]/[logSheetId]/page.tsx`                               | **1167** | Detail page (input + preview)            |
...
| A18 | `[projectId]/[logSheetId]/hooks/use-log-sheet-validation.ts`      |      202 | Client-side validation logic             |

### Features Layer (`src/features/log-sheets/`)

| #   | File                               |    Lines | Role                                   |
| --- | ---------------------------------- | -------: | -------------------------------------- |
| F1  | `actions.ts`                       |  **509** | Server actions (CRUD + business)       |
| F2  | `service.ts`                       | **1011** | Prisma service layer                   |
| F3  | `types.ts`                         |      186 | Zod schemas + interfaces               |
...
| F7  | `README.md`                        |        — | Documentation                          |

**Total: ~5,097 lines of code** (excluding README)
...
A4  → F3 (types)
A5  → A6
A6  → F3 (types), F1 (actions), @/features/users/actions, @/@types/user.type
...
All dependency flow is strictly unidirectional:

```
App Layer (A*) → Features Layer (F*) → Shared Libs (@/lib/*, @/@types/*)
```

However, there is one **cross-layer coupling concern**:
...
| ------------------------------ | -------: | :-----------------------------------: | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| **A7** `[logSheetId]/page.tsx` | **1167** |        1 component, 7 handlers        | 🔴 **GOD COMPONENT** — massive render function with inline table rendering for 3+ category types, mobile/desktop branching, and ~800 lines of JSX |
| **F2** `service.ts`            | **1011** | **17 exported functions** + 3 private | 🔴 **GOD MODULE** — handles CRUD, validation, machine mgmt, entries, photos, chemicals all in one file                                            |
| **F6** `log-sheet-preview.tsx` |  **713** |        1 component + 4 helpers        | 🟡 **LARGE** — complex print layout, but single responsibility                                                                                    |
| **F1** `actions.ts`            |  **509** |  **14 exported actions** + 2 private  | 🟡 **LARGE** — many actions but each is thin wrapper around service                                                                               |
...
- **A18** `use-log-sheet-validation.ts` — client-side validation
- **F2** `service.ts` L640-798 — `validateLogSheetForSubmission` + `validateLogSheetForApproval`
- Different implementations of the same business rules on client vs server.
...
graph TD
    ...
    subgraph "Features Layer: /features/log-sheets"
        F1["🟡 actions.ts<br/>(509L)"]
        F2["🔴 service.ts<br/>(1011L)"]
        F3["types.ts<br/>(186L)"]
        F4["utils.ts<br/>(38L)"]
