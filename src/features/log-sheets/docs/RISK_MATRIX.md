...
> Derived from [DEPENDENCY_MAP.md](file:///home/cursemaker/02_Projects/02_Freelance/01_corintek_cpis/src/features/log-sheets/DEPENDENCY_MAP.md) | 2026-02-19
...
| ------- | ---------------------------------------------------- | ----: | :--: | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **F2**  | `features/log-sheets/service.ts`                     |  1011 |  🔴  | **God module.** Core business layer for log sheets. All mutations and most reads flow through here, including status changes, locking checks, signature persistence, entries, photos, and chemicals. Coupled to Prisma, RBAC, parameters, machines, projects. |
...
| **A7**  | `[logSheetId]/page.tsx`                              |  1200 |  🔴  | **God component.** Orchestrates 7 hooks, 3 feature components (preview, signatures, chemical usage), and multiple server actions. ~900 lines of JSX with complex conditional tables and mobile/desktop branching. Single point for end-user behaviour.     |
...
| **F6**  | `components/log-sheet-preview.tsx`                   |   713 |  🟡  | **Print preview.** Large but single-purpose. Exports `CATEGORY_ORDER` consumed by A13 (cross-layer coupling). Contains its own copies of `formatLimit`, `formatRawWaterLimit`, and `machinesForCategory` logic used in app layer.                          |
...
| **A13** | `hooks/use-log-sheet-derived.ts`                     |   108 |  🟡  | **Derived data.** Computes categories, machines per category, and display names. Imports `CATEGORY_ORDER` from F6 (features-layer UI), creating cross-layer coupling. Duplicates `machinesForCategory` logic used by F6.                                   |
...
| :--------: | :----: | --------------------------------------------------------------------- |
|  🔴 HIGH   | **4**  | F2, F1, A7, F3                                                        |
| 🟡 MEDIUM  | **13** | A8, F4, A18, F6, A15, A13, A12, A16, A9, F9, F10, F11, F12            |
|   🟢 LOW   | **13** | A6, A10, A11, A14, A17, F5, F7, F8, A1, A2, A3, A4, A5                |

> **Interpretation:** The 4 HIGH-risk files still form the critical path for any log-sheet feature work, concentrating core business logic and coupling. The MEDIUM-risk group has grown with shared validation and status/locking helpers that must stay consistent with F2/F1 behaviour.
