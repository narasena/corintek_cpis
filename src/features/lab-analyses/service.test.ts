...
describe('Lab Analysis Service', () => {
  ...
  describe('getLabAnalysesByProject', () => {
    it('should fetch lab analyses for a project', async () => {
      ...
      vi.mocked(prisma.labAnalysis.findMany).mockResolvedValue(mockAnalyses as any);
  ...
  describe('createLabAnalysis', () => {
    it('should create a lab analysis with columns and entries', async () => {
      const input = {
        ...
        entries: [{ parameterId: 'param1', columnTempId: 'c1', textValue: 'value' }]
      };

      vi.mocked(prisma.labAnalysis.create).mockResolvedValue({ id: 'la-1' } as any);
      vi.mocked(prisma.labAnalysisColumn.create).mockResolvedValue({ id: 'real-c1', tempId: 'c1' } as any);
  ...
  describe('updateLabAnalysis', () => {
    it('should update an existing lab analysis and sync columns/entries', async () => {
      ...
      vi.mocked(prisma.labAnalysisColumn.create).mockResolvedValue({ id: 'real-new', tempId: 'c-new' } as any);

      const result = await updateLabAnalysis(input as any);

      expect(result.id).toBe('la-1');
      // Verify column deletion
      expect(prisma.labAnalysisColumn.updateMany).toHaveBeenCalledWith(expect.objectContaining({
