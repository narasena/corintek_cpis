...
describe('Users Service Characterization Tests', () => {
  ...
  describe('1. createUser', () => {
    ...
    it('throws specific error if user was soft-deleted', async () => {
      prismaMock.user.findFirst.mockResolvedValue(
        makeMockUser({ deletedAt: new Date() })
      );
  ...
  describe('2. updateUser', () => {
    ...
    it('updates user successfully', async () => {
      prismaMock.user.findUnique.mockResolvedValue(
        makeMockUser({ id: userId })
      );
      prismaMock.user.update.mockResolvedValue(
        makeMockUser({ id: userId, firstName: 'Jane' })
      );
    ...
    it('checks uniqueness when email is changed', async () => {
      ...
      prismaMock.user.findFirst.mockResolvedValue(
        makeMockUser({ id: '8f9c9c3e-8c3d-4c3e-8c3d-4c3e8c3d4c3e' })
      );

      await expect(
        updateUser(mockActor as any, userId, { email: 'new@example.com' })
  ...
  describe('3. deleteUser (Soft Delete)', () => {
    ...
    it('throws error if already deleted', async () => {
      ...
      await expect(deleteUser(mockActor as any, mockActor.id)).rejects.toThrow(
  ...
  describe('5. getTechniciansList', () => {
    it('returns list of technicians if authorized', async () => {
      prismaMock.user.findMany.mockResolvedValue([
        ...
        makeMockUser({
          id: '8f9c9c3e-8c3d-4c3e-8c3d-4c3e8c3d4c3e',
          role: 'TECHNICIAN',
        }),
      ] as any);
  ...
  describe('7. getUserById', () => {
    ...
    it('throws error if user not found', async () => {
      ...
      await expect(getUserById(mockActor as any, userId)).rejects.toThrow(
    ...
    it('throws error if user is deleted', async () => {
      prismaMock.user.findUnique.mockResolvedValue(
        makeMockUser({ id: userId, deletedAt: new Date() })
      );
      await expect(getUserById(mockActor as any, userId)).rejects.toThrow(
        'Pengguna tidak ditemukan'
      );
  ...
  describe('8. permanentlyDeleteUser', () => {
    ...
    it('throws if user not found', async () => {
      prismaMock.user.findUnique.mockResolvedValue(null);
      await expect(permanentlyDeleteUser(userId)).rejects.toThrow(
        'Pengguna tidak ditemukan'
      );
    });
  });
  ...
  describe('9. isUserAuthValid (Utils)', () => {
    it('returns true for valid active user', () => {
      expect(
        isUserAuthValid({ deletedAt: null, isActive: true, isBlocked: false })
      ).toBe(true);
    });
    ...
    it('returns false if inactive', () => {
      expect(
        isUserAuthValid({ deletedAt: null, isActive: false, isBlocked: false })
      ).toBe(false);
    });

    it('returns false if blocked', () => {
      expect(
        isUserAuthValid({ deletedAt: null, isActive: true, isBlocked: true })
      ).toBe(false);
    });
  });
});
