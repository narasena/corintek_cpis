...
test.describe('M-03: Shared Infrastructure & Components E2E', () => {
  test.describe('CUJ-01: RBAC Shield & Navigation', () => {
    test('Technician should not see Admin links and be redirected if accessing /users', async ({
      page,
    }) => {
  ...
  test.describe('CUJ-02: DataTable Tab & RBAC Integration', () => {
    test.skip('DataTable should switch tabs and update data correctly', async ({
      page,
    }) => {
  ...
  test.describe('CUJ-03: Secure Media Capture Flow', () => {
    test('CameraInput should process uploaded image to WebP 1:1', async ({
      page,
    }) => {
      await loginAsTechnician(page);
      ...
      if (!(await uploadButton.isVisible())) {
        console.log(
          'Skipping CUJ-03 as CameraInput is not visible (user might be clocked out)'
        );
        return;
      }
